...
const { selectedCard, getSelectedCardId } = storeToRefs(selectedCardStore());
const { fetchBook, clearSelectedCard } = selectedCardStore();
...
const descriptionHandler = computed(() => typeof selectedCard.value?.description === 'string' ? selectedCard.value?.description : selectedCard.value?.description?.value);

onMounted(async () => {
  // по идее можно просто хранить selectedCard в localStorage и брать оттуда, 
  // как и вообще всё избранное, но так лень господи прости грешного
  if (!getSelectedCardId.value) return router.push('/search');
  await fetchBook();
})
    ...
    <acticle v-else class="book-card" key="2">
      ...
      <section class="book-card__content">
        ...
        <section class="book-card__info">
          ...
          <div v-else class="book-card__info-author">Авторы: 
            <span v-for="(author, i) in selectedCard.authorName.slice(0, authorsShowLimit)" :key="i">
              <span class="__bold">{{ author }}</span><span v-if="i !== authorsShowLimit">,</span>
            </span>
          </div>
          ...
          <button @click="isFav ? removeFromFavs(getFavBookIndex(selectedCard)) : addToFavs(selectedCard)" 
      ...
      <section v-if="selectedCard.subjects?.length" class="book-card__tags-content">
        ...
        <div class="book-card__tags">
          <span v-for="(subject, i) in selectedCard.subjects?.slice(0, tagsShowLimit)" 
              :key="i" 
